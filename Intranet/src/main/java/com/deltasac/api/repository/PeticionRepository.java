...
public interface PeticionRepository extends JpaRepository<Peticion, Integer> {
	...
	@Query("SELECT p FROM Peticion p WHERE p.idarea=?1 AND p.estado<>'FINALIZADA'")
	public List<Peticion> buscarPeticionPorArea(Integer idarea);
	
	@Query("SELECT p FROM Peticion p WHERE p.idasignado=?1 AND p.estado<>'FINALIZADA'")
	public List<Peticion> buscarPeticionPorAsignado(Integer idasignado);
	
	@Query("SELECT p FROM Peticion p WHERE p.idusuario=?1 AND p.estado<>'FINALIZADA'")
